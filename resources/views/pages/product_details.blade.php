					...
					<div class="tab-pane fade active in" id="reviews" >
						<div class="col-sm-12">
							...
							<form action="{{url('/review-rating')}}" method="post" >
								...
								<span>
									<input type="name" name="reviewer_name" placeholder="Your Name" required="" />
									<input type="email" name="reviewer_email" placeholder="Email Address" required="" />
								</span>
							...
							<h3>Review</h3>
							<?php
								$all_review=DB::table('tbl_review')
								->where('product_id',$product_by_details->product_id )
								->get();
							?>
							@foreach($all_review as $show_review)
								<p><b>{{$show_review->reviewer_name}}</b><br><i>{{$show_review->review}}</i></p>
								<br>
